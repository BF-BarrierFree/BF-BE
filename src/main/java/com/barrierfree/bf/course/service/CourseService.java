...
public class CourseService {
  ...
  public CourseResponse createCourse(Long userId, CourseCreateRequest request) {
    ...
    Course course = Course.builder().user(user).title(request.title()).isAiGenerated(false).build();
  ...
  public CourseResponse getCourseDetail(Long userId, Long courseId) {
    Course course =
        courseRepository
            .findByIdAndUserIdWithPlaces(courseId, userId)
    ...
    for (int i = 0; i < savedPlaceIds.size(); i++) {
      ...
      SavedPlace savedPlace =
          savedPlaceRepository
              .findByIdAndPlaceListUserId(placeId, userId)
      ...
      if (i < savedPlaceIds.size() - 1) {
        ...
        SavedPlace nextPlace =
            savedPlaceRepository
                .findByIdAndPlaceListUserId(nextPlaceId, userId)
                .orElseThrow(() -> new CustomException(ErrorCode.SAVED_PLACE_NOT_FOUND));
  ...
  private String calculateDistanceToNext(SavedPlace current, SavedPlace next) {
    try {
      WheelchairRouteResponse routeResponse =
          orsRouteService.getWheelchairRoute(
