...
public class AiCourseGenerateService {
  ...
  public AiCoursePreviewResponse generateCoursePreview(AiCourseGenerateRequest request) {
    log.info(
        "AI 코스 생성 요청 수신: 지역={}, 테마={}, 일정={}",
        request.region().getLabel(),
        request.theme().getLabel(),
        request.duration().getLabel());
    ...
    for (String rule : compositionRule) {
      PlaceSearchResponse.PlaceSummary selectedPlace =
          findBestPlaceForRule(rule, request, usedPlaceIds);
