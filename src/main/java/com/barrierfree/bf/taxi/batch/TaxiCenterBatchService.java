...
public class TaxiCenterBatchService {
  ...
  public void fetchAndUpsertTaxiCenters() {
    ...
    while (hasNext) {
      try {
        ...
        String requestUri =
            UriComponentsBuilder.fromUriString(taxiCenterBaseUrl)
                .path("/center_info_v2")
        ...
        String jsonResponse =
            tagoWebClient.get().uri(requestUri).retrieve().bodyToMono(String.class).block();
        ...
        JsonNode bodyNode =
            rootNode.has("response") ? rootNode.get("response").get("body") : rootNode.get("body");

        if (bodyNode == null || !bodyNode.has("item")) {
          break;
        }

        TagoTaxiCenterResponse.Body body =
            objectMapper.treeToValue(bodyNode, TagoTaxiCenterResponse.Body.class);
        ...
        Map<String, TaxiCenter> existingCenterMap =
            existingCentersList.stream().collect(Collectors.toMap(TaxiCenter::getCenterId, c -> c));
        ...
        for (TagoTaxiCenterResponse.CenterItem item : body.getItem()) {
          try {
            ...
            log.error(
                "[Batch] Center ID: {} 처리 실패. 다음 센터로 진행합니다: {}", item.getCntrId(), e.getMessage());
          }
        }
  ...
  private TaxiCenter processTaxiCenter(
      TagoTaxiCenterResponse.CenterItem item, TaxiCenter existingCenter) {
    ...
    boolean requiresNewSchema =
        existingCenter == null
            || (existingCenter.getReferenceDate() == null
                || !existingCenter.getReferenceDate().equals(item.getTotCrtrYmd()))
    ...
    if (requiresNewSchema) {
      ...
      try {
        formSchema =
            openRouterService.generateFormSchema(item.getUtztnTrgtExpln(), item.getRsvtGdMttr());
      } catch (Exception e) {
        log.warn(
            "[Batch] Center ID: {} 폼 스키마 생성 실패. 기본 빈 스키마로 적재합니다: {}",
            item.getCntrId(),
            e.getMessage());
        formSchema = "[]";
      }
