...
public class TaxiReservationService {
    ...
    public List<TaxiCenterResponse> findNearestCenters(double userLat, double userLng, double maxDistanceKm, int limit) {
        List<TaxiCenter> allCenters = taxiCenterRepository.findAll();

        return allCenters.stream()
                .map(center -> {
                    double distance = calculateHaversineDistance(userLat, userLng, center.getLatitude(), center.getLongitude());
    ...
    public String estimateFare(TaxiReservationRequest.FareEstimate request) {
        TaxiCenter center = taxiCenterRepository.findById(request.getCenterId())
     ...
     */
    @Transactional
    public void saveReservationHistory(Long userId, TaxiReservationRequest.CreateReservation request) {
        ...
        TaxiReservation reservation = TaxiReservation.builder()
                .user(user)
                .taxiCenter(center)
