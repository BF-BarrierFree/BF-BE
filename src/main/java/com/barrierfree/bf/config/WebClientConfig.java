...
public class WebClientConfig {
    ...
    /** 
     * 비동기 HTTP 통신을 위한 기본 WebClient 빈(Bean) 등록 
     * 서비스 계층(Service)에서 주입받아 구글 API 등을 호출할 때 사용합니다. 
     */
    @Bean
    ...
    /**
     * 공공데이터포털(TAGO) API 호출 전용 WebClient 빈(Bean). 
     * 공공데이터포털 인증키는 이미 인코딩된 상태로 발급되는 경우가 많아, 
     * WebClient가 요청을 보낼 때 URL을 한 번 더 인코딩(Double Encoding)하는 것을 방지합니다.
     */
    @Bean
    ...
    /**
     * 외부 LLM (OpenRouter) API 호출 전용 WebClient 빈(Bean)
     * LLM 생성 응답은 시간이 더 걸릴 수 있으므로 타임아웃을 넉넉히 설정합니다.
     */
    @Bean
